# Oviya Jeyaprakash, 5/20/2023 
# Perceptrons Graphing
# l930
...
for n in range(16):
    ...
    result = training(n, w, b, (t:= truth_table(bits, n)), 0)
    #print(t)
    ...
    for i in np.arange(-2.0, 2.0, 0.1):
        for j in np.arange(-2.0, 2.0, 0.1):
            c = ""
            if perceptron(step, result[1], result[2], (i, j)) == 0:
                c = "red"
            else:
                c = "green"
            data_list_x.append(i)
            data_list_y.append(j)
            data_list_color.append(c)
            
    axs[(n//4), (n%4)].scatter(data_list_x, data_list_y, color = data_list_color, s = 5)
    ...
    for key, value in t.items():
        ...
        if value == 0:
            ...
            c = "green"
        axs[(n//4), (n%4)].scatter(key[0], key[1], color = c, s = 10)
