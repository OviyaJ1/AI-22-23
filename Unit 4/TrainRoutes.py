    ...
    while fringe:
        ...
        if v[1] == city_end_id:
            ...
            while count <= (len(closed[v[1]][1]) - 2):
                c.itemconfig(line_dict_acc[(closed[v[1]][1][count], closed[v[1]][1][count + 1])], fill="green")
                if count % 100:
                    r.update()
                count += 1
            return v[0]
        if v[1] not in closed_acc:
            ...
            for child in junctions_dict[v[1]]:
                if child[0] not in closed_acc:
                    c.itemconfig(line_dict_acc[(child[0], v[1])], fill="red")
    ...
    while fringe:
        ...
        if v[2] == city_end_id:
            ...
            while count <= (len(closed[v[2]][1]) - 2):
                c.itemconfig(line_dict_acc[(closed[v[2]][1][count], closed[v[2]][1][count + 1])], fill="green")
                if count % 100:
                    r.update()
                count += 1
            return v[1]
        if v[2] not in closed_acc:
            ...
            for child in junctions_dict[v[2]]:
                if child[0] not in closed_acc:
                    c.itemconfig(line_dict_acc[(child[0], v[2])], fill="red")
                    if count_update % 1000 == 0:
                        r.update()
...
def draw(r, c):
    for key in line_dict_draw.keys():
        c.itemconfig(line_dict_draw[key], fill = "black")
        r.update()
